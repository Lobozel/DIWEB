...
(function($) {
  var Item = function(image, mirrorOptions) {
    ...
    this.moveTo = function(x, y, scale) {
      ...
      } else style.height = this.height + "px";
    };
  };
  ...
  var Carousel = function(container, options) {
    ...
    this.items = [];
    this.xCentre =
      options.xPos === null ? container.width() * 0.5 : options.xPos;
    this.yCentre =
      options.yPos === null ? container.height() * 0.1 : options.yPos;
    this.xRadius =
    ...
    this.play = function() {
      if (this.frameTimer === 0) this.scheduleNextFrame();
    };
      ...
      if (options.bringToFront) {
        container.bind("click.cloud9", function(event) {
          ...
          if (hits.length !== 0) {
            ...
            if (hits[0].style.zIndex >= 99) {
              window.location = "especies/" + event.target.id + ".html";
            }
    ...
    this.finishInit = function() {
      ...
      for (var i = 0; i < images.length; i++) {
        ...
        if (
          im.width === undefined ||
          (im.complete !== undefined && !im.complete)
        )
          return;
      }
